...
cfg.dip.pos     = dip.pos;      %leadfield.pos([2319],:); % I chose these points manualy 
cfg.dip.mom     = dip.mom;      %[10 0 0]';     % note, it should be transposed
% cfg.dip.frequency = 10;
cfg.dip.amplitude = 70;
cfg.dip.unit    = 'cm';
cfg.ntrials     = 1;
cfg.triallength = 4;            % seconds
cfg.fsample     = 250;          % Hz
time = (1:250)/250;
signal1 = 70*sin(10*time*2*pi);
signal2 = 100*cos(20*time*2*pi);
signal3 = 90*cos(30*time*2*pi);
cfg.dip.signal = {[signal1; signal2;signal3]};
...
ft_plot_dipole(dip.pos_scaled(1,:), dip.mom(1,:),...
    ...
    'color', 'red',...
    'unit', 'cm');
ft_plot_dipole(dip.pos_scaled(2,:), dip.mom(2,:),...
    'diameter',5,...
    'length', 10,...
    'color', 'red',...
    'unit', 'cm');
ft_plot_dipole(dip.pos_scaled(3,:), dip.mom(3,:),...
    'diameter',5,...
    'length', 10,...
    'color', 'red',...
    'unit', 'cm');
...
simEEG.trials = 1;
simEEG.event = 1;
simEEG.chanlocs = elec_aligned.chanpos;
simEEG.nbchan = 64;
simEEG.srate = 250;
simEEG.xmin = 0;
simEEG.xmax = 1;
